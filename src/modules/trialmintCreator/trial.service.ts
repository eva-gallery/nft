...
export class TrialCreator {
    ...
    async createTrialCall(
        ...
        const call = await this.nftCreatorInstance.createNFTcall(collectionID, metadataCid.path);
        ...
        return new Promise<string>((resolve, reject) => {
            api.tx(call).signAndSend(EvaGallerySigner, async ({ status, dispatchError }) => {
                if (status.isFinalized) {
                    if (dispatchError) {
                        ...
                        resolve(JSON.stringify({ nftID, metadataPath: metadataCid.path, cidPath: cid.path }));
                    }
                }
            });
        });
