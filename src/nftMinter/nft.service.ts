...
const createNFT = (api: ApiPromise, collectionID: string, nftID: string, owner: string) => {
    ...
    export class nftCreator {
       ...
       async createNFTcall(nft: NftDto): Promise<any> {
        ...
        const calls: SubmittableExtrinsic<'promise'>[] = [createNFT(api, collectionID.toString(), nextNFT.toString(), address)];
        if (metadata) {
          calls.push(setNFTMetadata(api, collectionID.toString(), nextNFT.toString(), JSON.stringify(metadata)));
        }
